...
async function getClosestMatch(CurrentDate) {
  const closest_match = await DButils.execQuery(
    // `SELECT TOP 1 match_id, MIN(Matches.match_hour) FROM Matches WHERE Matches.match_hour>='${CurrentDate}' GROUP BY Matches.match_hour ORDER BY Matches.match_hour`

    `SELECT TOP 1 match_id FROM Matches WHERE Matches.match_hour>='${CurrentDate}' ORDER BY Matches.match_hour`
  );
...
async function getMatchPreviewById(match_id) {
  const preview = await DButils.execQuery(
    `SELECT match_id,match_date,match_hour,home_team,away_team,referee_id,stadium FROM Matches WHERE Matches.match_id ='${match_id}'`
  );
  if (preview)
